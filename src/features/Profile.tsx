...
export function Profile({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col bg-canvas">
      <ScreenHeader title="Profile" onBack={onBack} right={<button onClick={() => nav.push("settings")} className="text-[20px]">⚙️</button>} />
      <Scroll className="px-5 py-4">
        <div className="flex items-center gap-3">
          <Avatar initials={journey.initials} color={journey.avatarColor} size={60} />
          <div>
            <h2 className="text-[19px] font-bold text-ink">{journey.name}, {journey.age}</h2>
        ...
        {tab === "saved" && (
          saved.length === 0 ? (
            <EmptyState icon="🔖" title="Nothing saved yet" body="Save places, Lens insights and phrases and they'll collect here for offline reference." />
          ) : (
            <div className="space-y-2">
              {saved.map((s) => (
                <div key={s.id} className="flex items-center gap-3 rounded-[14px] bg-surface px-4 py-3 shadow-card">
                  <span className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-canvas text-[16px]">{s.icon ?? "🔖"}</span>
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        ...
        <div className="flex gap-2">
          {asean.stamps.map((s) => (
            <div key={s.code} className="flex flex-col items-center rounded-[16px] border border-dashed border-primary/30 bg-primary-soft/40 px-4 py-3">
...
export function Settings({ onBack }: { onBack: () => void }) {
  const { setForced, forced } = useJourney();
  return (
    <div className="flex h-full flex-col bg-canvas">
      <ScreenHeader title="Settings" onBack={onBack} />
      <Scroll className="px-5 py-4">
        <Card className="p-4">
          <p className="text-[13px] font-bold text-ink">🎬 Demo: force UI state</p>
          <p className="mt-0.5 text-[12px] text-muted">Applies across Today, Passport, Lens, Explore & Connect.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            {STATES.map((s) => (
              <button key={s.key} onClick={() => setForced(s.key)} className={`rounded-full border px-3 py-1.5 text-[12px] font-semibold ${forced === s.key ? "border-primary bg-primary text-white" : "border-line bg-surface text-muted"}`}>{s.label}</button>
            ))}
          </div>
        </Card>

        <div className="mt-4 space-y-1">
          {["Notifications", "Privacy & data", "Saved insights", "Language", "Help & support", "Log out"].map((r) => (
            <button key={r} className="flex w-full items-center justify-between rounded-[14px] bg-surface px-4 py-3.5 text-left text-[14px] font-medium text-ink shadow-card active:scale-[.99]">
              {r} <Icon name="chevron" size={16} />
            </button>
          ))}
        </div>
      </Scroll>
      ...
      <Scroll className="px-5 py-4">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between gap-2">
            <button onClick={() => setPick("home")} className={`flex flex-1 flex-col items-center rounded-[16px] border-2 py-3 active:scale-[.98] ${pick === "home" ? "border-primary bg-primary-soft" : "border-line bg-surface"}`}>
            ...
            <button onClick={() => setPick("host")} className={`flex flex-1 flex-col items-center rounded-[16px] border-2 py-3 active:scale-[.98] ${pick === "host" ? "border-primary bg-primary-soft" : "border-line bg-surface"}`}>
        ...
        <div className="space-y-2.5">
          {JOURNEYS.map((j) => {
            ...
            return (
              <button key={j.id} onClick={() => { setJourneyId(j.id); onBack(); }} className={`flex w-full items-center gap-3 rounded-[16px] border px-4 py-3 text-left active:scale-[.99] ${active ? "border-primary bg-primary-soft" : "border-line bg-surface"}`}>
                ...
                {j.deep && !active && <Badge tone="amber">★ Deep</Badge>}
              </button>
            );
          })}
        </div>

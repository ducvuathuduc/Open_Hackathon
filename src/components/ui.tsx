...
export function Button({
  ...
  children: ReactNode;
  onClick?: () => void;
  variant?: "primary" | "amber" | "ghost" | "soft" | "danger" | "outline";
  full?: boolean;
  size?: "md" | "lg" | "sm";
  disabled?: boolean;
  className?: string;
  ...
  const base =
    "inline-flex items-center justify-center gap-2 font-semibold rounded-[14px] transition active:scale-[.98] disabled:opacity-40 disabled:active:scale-100";
  const sizes = {
    sm: "text-[13px] px-3 min-h-[40px]",
    md: "text-[15px] px-4 min-h-[46px]",
    lg: "text-[16px] px-5 min-h-[52px]",
  };
  const variants = {
    primary: "bg-primary text-white shadow-[0_6px_16px_rgba(49,87,213,.28)]",
    amber: "bg-amber text-ink shadow-[0_6px_16px_rgba(255,182,72,.32)]",
...
export function Chip({
  ...
  const tones = {
    ...
    primary: active ? "bg-primary text-white border-primary" : "bg-primary-soft text-primary border-transparent",
    amber: active ? "bg-amber text-ink border-amber" : "bg-amber-soft text-ink border-transparent",
  };
  return (
    <button
      onClick={onClick}
      className={`shrink-0 rounded-full border px-3.5 py-2 text-[13px] font-medium transition active:scale-95 min-h-[36px] ${tones[tone]}`}
    >
      {children}
    </button>
  );
...
export function Segmented<T extends string>({
  ...
  return (
    <div className="flex gap-1 rounded-[14px] bg-canvas p-1">
      {options.map((o) => (
        <button
          key={o.value}
          onClick={() => onChange(o.value)}
          className={`flex-1 rounded-[11px] px-2 py-2 text-[13px] font-medium transition min-h-[40px] ${
...
export function Card({
  ...
  return (
    <div
      onClick={onClick}
      style={style}
      className={`rounded-[20px] bg-surface shadow-card ${onClick ? "cursor-pointer active:scale-[.99] transition" : ""} ${className}`}
    >
      {children}
    </div>
  );
...
export function SectionHeader({ title, action, onAction }: { title: string; action?: string; onAction?: () => void }) {
  return (
    <div className="mb-3 flex items-center justify-between">
      ...
      {action && (
        <button onClick={onAction} className="text-[13px] font-semibold text-primary">
          {action}
        </button>
      )}
    </div>
...
export function CountryPairChip({ home, host, onClick }: { home: CountryCode; host: CountryCode; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1.5 rounded-full bg-surface/90 px-3 py-1.5 text-[13px] font-semibold text-ink shadow-card backdrop-blur"
    >
...
export function ProgressBar({ value, tone = "primary" }: { value: number; tone?: "primary" | "amber" | "success" }) {
  const colors = { primary: "bg-primary", amber: "bg-amber", success: "bg-success" };
  return (
...
export function Badge({ children, tone = "muted" }: { children: ReactNode; tone?: "muted" | "primary" | "success" | "warning" | "error" | "amber" }) {
  const tones = {
...
export function RiskBadge({ value }: { value: number }) {
  ...
  const emoji = value >= 60 ? "🔴" : value >= 40 ? "🟠" : "🟢";
  return <Badge tone={tone}>{emoji} {value}% risk</Badge>;
}
...
export function ScoreBarRow({ label, value, delta }: { label: string; value: number; delta?: number }) {
  const tone = value >= 80 ? "bg-success" : value >= 60 ? "bg-primary" : "bg-warning";
  return (
    <div>
      <div className="mb-1 flex items-baseline justify-between">
        <span className="text-[13px] font-medium text-ink">{label}</span>
...
export function Skeleton({ className = "" }: { className?: string }) {
  return <div className={`yy-skeleton rounded-[14px] ${className}`} />;
}
...
export function ErrorState({ onRetry }: { onRetry?: () => void }) {
  return (
    <EmptyState icon="⚠️" title="Something went wrong" body="We couldn't load this right now. Please try again." action="Retry" onAction={onRetry} />
  );
}

export function OfflineBanner() {
  return (
    <div className="flex items-center gap-2 rounded-[14px] bg-ink px-3 py-2.5 text-[13px] font-medium text-white">
      <span>📡</span> You're offline — showing saved content
    </div>
  );
}

export function Notice({ tone = "warning", icon, title, body }: { tone?: "warning" | "error" | "primary"; icon: string; title: string; body: string }) {
  const tones = {
  ...
  return (
    <div className={`rounded-[16px] border px-3.5 py-3 ${tones[tone]}`}>
...
export function BottomSheet({ open, onClose, title, children }: { open: boolean; onClose: () => void; title?: string; children: ReactNode }) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-end" onClick={onClose}>
      ...
      <div className="yy-fade relative w-full rounded-t-[24px] bg-surface p-5 pb-7 shadow-pop" onClick={(e) => e.stopPropagation()}>

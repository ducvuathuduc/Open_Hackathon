...
export async function setTaskProgress(userId: string, taskId: string, done: boolean): Promise<void> {
  ...
  try {
    ...
    const data = { user_id: userId, task_id: taskId, status: done ? "done" : "pending", completed_at: done ? new Date().toISOString() : null };
    if (existing.rows[0]) {
      await tablesDB.updateRow({ databaseId: APPWRITE_DATABASE_ID!, tableId: TABLE_ID, rowId: existing.rows[0].$id, data });
    } else {
      await tablesDB.createRow({ databaseId: APPWRITE_DATABASE_ID!, tableId: TABLE_ID, rowId: ID.unique(), data });
    }
  } catch {
